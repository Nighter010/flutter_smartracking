...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check if the record with the given id_card already exists
    $sqlCheck = "SELECT * FROM health_db WHERE id_card = '$id_card'";
    $resultCheck = mysqli_query($conn, $sqlCheck);

    if (mysqli_num_rows($resultCheck) > 0) {
        // If the record exists, update it
        $sqlUpdate = "UPDATE health_db SET titlename='$titlename', firstname='$firstname', lastname='$lastname', 
        ...
        // Send error response code - Record not found
        http_response_code(404);
        echo json_encode(array("message" => "Record not found for the provided id_card"));
    }
} else {
